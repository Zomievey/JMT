$(document).ready(function () {
    ...
    var varterIndex = 0;
    var currentvarterDiv = $("#next-varter");
    currentvarterDiv.text(currentvarter);
    $("#words").append(sentences[sentenceIndex]);

    var currentSentence = sentences[0];
    var currentvarter = currentSentence[0];
    var start; // for timer
    ...
    $("#next-varter").append(currentvarter); // setting the content of the next-varter div to = currentvarter
    ...
    $(document).keypress(function (event) {
        ...
        var currentvarter = currentSentence[varterIndex];
        ...
        varterIndex++; //advances the varterIndex by 1
        var nextvarter = currentSentence[varterIndex]; // sets nextvarter = the current varterIndex
        currentvarterDiv.text(nextvarter); //sets the content of currentvarterDiv = nextvarter

        if (varterIndex < currentSentence.length -1) { // if varterIndex hasn't gone through the entire word yet
            if (event.which === currentvarter.charCodeAt()) { // if key pressed is correct
        ...
        if (varterIndex == currentSentence.length) { // if varterIndex = the end of the currentSentence
            ...
            varterIndex = 0; //reset the varter index to 0 to start at the beginning of the new sentence
            if (sentenceIndex < sentences.length - 1) { 
                var nextvarter = currentSentence[varterIndex];
            }
            currentvarterDiv.text(nextvarter); // set the content of the currentvarterDiv = nextvarter
        ...
        if (sentenceIndex > sentences.length - 1) { //if you're all the way through the sentences array
            ...
            $("#next-varter").text("Your score is " + speed + " words per minute"); //setting content of next-varter div to results of test
